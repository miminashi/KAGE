...
void combineHame2(const KGString *parts1, const KGString *parts3, int *result){
	...
	for(i = 0; i < strokes; i++){
		if(buf[i * 11 + 0] == 9){
			result[8] = buf[i * 11 + 3];
			if(flg_box & FLAG_FLAT_LEFT) result[8] += kWidth * 4;
			result[9] = buf[i * 11 + 4];
			if(flg_box & FLAG_FLAT_TOP) result[9] += kWidth * 4;
			result[10] = buf[i * 11 + 5];
			if(flg_box & FLAG_FLAT_RIGHT) result[10] -= kWidth * 4;
			result[11] = buf[i * 11 + 6];
			if(flg_box & FLAG_FLAT_BOTTOM) result[11] -= kWidth * 4;
			flag = 1;
		}
	}
	if(flag == 0){ //error
